...
exports.add = function (actor, scope) {
  if (utils.aid.isBare(actor.id)) actor.id += '/' + utils.uuid();
  ...
  if (utils.aid.isBare(aid)) {
    ...
    if (bares) {
      var aids = _.filter(bares, function (item) {
        return exports.inScope(item, scope);
      });
      ...
      logger.makeLog('trace', 'hub-44', 'cant remove bare actor ' + aid + ': no matching full aid');
    }
  } else {
    ...
    if (actor) {
      ...
      logger.makeLog('trace', 'hub-34', 'cant remove non existing actor ' + aid);
    }
  }
...
/**
 * Get all actors in cache matching with given aid (all full aid matching given aid if bare)
 * @param reqAid {string} requested aid
 * @param [scope] {string} requested scope
 * @returns {Array} matching aid collection
 */
exports.getAll = function (reqAid, scope) {
  scope = scope || exports.scope.ALL;
  var aids = [];
...
exports.exists = function (reqActor, scope) {
  if (!_.isString(reqActor) && !_.isObject(reqActor)) return false;
