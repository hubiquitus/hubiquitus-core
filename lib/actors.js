...
var logger = require('./logger')('hubiquitus:core');
var utils = {
...
exports.remove = function (aid) {
  if (utils.aid.isBare(aid)) {
    _.forOwn(bareActors[aid], function (fullAids) {
      _.forEach(fullAids, function (item) {
        delete actors[item];
      });
    });
    ...
    if (actor) {
      ...
      _.forEach([actor.scope, exports.scope.ALL], function (scope) {
        _.remove(bareActors[bare][scope], function (currentAid) {
          return currentAid === aid;
        });
      });
      if (_.isEmpty(bareActors[bare][exports.scope.ALL])) h.events.emit('actor removed', bare);
      delete actors[aid];
    } else {
      logger.makeLog('trace', 'hub-34', 'actor ' + aid + ' removal failed : does\'nt exist');
    }
  }
...
exports.pick = function (reqAid) {
  ...
  if (utils.aid.isFull(reqAid)) {
    ...
    if (bareAids) {
      ...
      aid = utils.misc.pickOne(matchingAids);
    }
  }
  return aid;
};
