...
exports.pick = function (reqAid) {
  var aid;
  if (utils.aid.isFull(reqAid) && actors[reqAid] && actors[reqAid].container.id === h.ID) {
    aid = reqAid;
  } else {
    var matchingActors = [];
    _.forEach(_.keys(actors), function (currentAid) {
      if (utils.aid.bare(currentAid) === reqAid && actors[currentAid].container.id === h.ID) {
...
exports.pickAll = function (reqAid) {
  var aids = [];
  if (utils.aid.isFull(reqAid) && aids[reqAid] && aids[reqAid].container.id === h.ID) {
    aids.push(reqAid);
  } else {
    _.forEach(_.keys(aids), function (currentAid) {
      if (utils.aid.bare(currentAid) === reqAid && aids[currentAid].container.id === h.ID) {
