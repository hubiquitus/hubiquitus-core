...
exports.search = function (aid, callback) {
  ...
  if (events.listeners(aid + "!found").length === 0) {
    ...
    logger.makeLog("trace", "hub-20", {msg: "discovery on " + aid + " already in progress...", ID: h.ID});
  }
};
...
function onDiscoveryResult(message) {
  logger.makeLog("trace", "hub-9", {msg: "actors " + message.aids + " found on node " + message.from, ID: h.ID}, message);
