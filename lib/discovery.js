 ...
 */
exports.notifySearched = function (aid, cb) {
...
function internalSearch(aid) {
  ...
  if (discoveryCountsPerAid[aid] === properties['max discoveries per aid']) {
  ...
  if (localAid) {
    ...
    var discoveryMsg = encode({type: 'search', from: h.ID, aid: aid, netInfo: localInfos});

    if (mcastSock) {
      localSock.send(discoveryMsg, 0, discoveryMsg.length, mcastInfos.port, mcastInfos.host);
    } else if (discoveryAddrs) {
      _.forEach(discoveryAddrs, function (container) {
        localSock.send(discoveryMsg, 0, discoveryMsg.length, container.port, container.host);
      });
     }
  }
...
function onMessage(buffer) {
  ...
  if (msg && msg.type === 'search' && msg.from !== h.ID) {
    onSearch(msg);
  } else if (msg && msg.type === 'result' && msg.to === h.ID) {
    onResult(msg);
  }
}
...
function onSearch(msg) {
  ...
  if (!_.isEmpty(aids)) {
    ...
    _.forEach(aids, function (aid) {
      var response = encode({type: 'result', from: h.ID, to: msg.from, aid: aid, netInfo: h.netInfo});
...
function onResult(msg) {
  ...
  events.emit(msg.aid + '!found', msg.aid);
  events.emit(utils.aid.bare(msg.aid) + '!found', msg.aid);
}
