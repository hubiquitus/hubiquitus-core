 ...
 */
var properties = {
  'discovery timeout': 5 * 60000,
  'min discovery interval': 18000,
  'max discovery interval': 22000
};
...
exports.notifySearched = function (aid) {
  if (!lastResearches[aid]) {
    (function tick(aid) {
      ...
      exports.search(aid);
      setTimeout(function () {
        if ((new Date()).getTime() - lastResearches[aid] < properties['discovery timeout']) {
          tick(aid);
        } else {
          logger.makeLog('trace', 'hub-12', 'stop discovery for ' + aid + '; too much time since last request');
          delete lastResearches[aid];
        }
      }, _.random(properties['min discovery interval'], properties['max discovery interval']));
    })(aid);
  }
 ...
 * @params {string} aid
 */
exports.search = function (aid) {
  logger.makeLog('trace', 'hub-10', 'searching actor ' + aid + '...');
  var msg = encode({type: 'search', from: h.ID, aid: aid, netInfo: localInfos});
