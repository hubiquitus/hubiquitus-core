 ...
 * - discovery timeout : if an actor has not been request for that, its discovery stops
 * - min discovery interval : min delay between two discoveries related to an actor
 * - min discovery interval : max delay between two discoveries related to an actor
 */
var _properties = {
...
exports.notifySearched = function (aid) {
  if (!lastResearches[aid]) {
    ...
    (function tick(aid) {
      ...
      exports.search(aid);
      setTimeout(function () {
        if ((new Date()).getTime() - lastResearches[aid] < _properties['discovery timeout']) {
          tick(aid);
        } else {
          logger.makeLog('trace', 'hub-12', 'stop discovery for ' + aid + '; too much time since last request');
          delete lastResearches[aid];
        }
      }, _.random(_properties['min discovery interval'], _properties['max discovery interval']));
    })(aid);
  } else {
    lastResearches[aid] = (new Date()).getTime();
  }
