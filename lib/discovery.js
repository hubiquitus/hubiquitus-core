...
function internalSearch(aid) {
  var discoveryMsg = encodeDiscoveryMsg({type: "search", from: h.ID, aid: aid, sender:localInfos});
 ...
 * @param {Buffer} buffer
 */
function onDiscoveryMessage(buffer, infos) {
...
function encodeDiscoveryMsg(msg) {
  ...
  if (msg.type === "search") {
    ...
    var senderHost = new Buffer(msg.sender.host, "utf8");
    var senderPort = new Buffer(2);
    senderPort.writeUInt16BE(msg.sender.port, 0);
