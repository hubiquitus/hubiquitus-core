 ...
 * @param {string} multicastAddr sample "udp://224.0.0.1:5555"
 * @param {number} localPort used to receive discovery request or results
...
exports.search = function (aid, callback) {
  if (localSocket) {
    ...
    logger.makeLog("trace", "hub-23", "No discovery addr nor discovery port provided. Search canceled.");
  }
};
...
function internalSearch(aid) {
  var discoveryMsg = encodeDiscoveryMessage({type: "search", from: h.ID, aid: aid, sender:localInfos});
...
function onDiscoverySearch(message) {
  ...
  if (!_.isEmpty(aids)) {
    ...
    _.forEach(aids, function (aid) {
      ...
      localSocket.send(response, 0, response.length, message.sender.port, message.sender.host);
    });
  }
}
...
function encodeDiscoveryMessage(message) {
  ...
  if (message.type === "search") {
    ...
    var from = new Buffer(message.from, "utf8");
    var aid = new Buffer(message.aid, "utf8");
    var senderHost = new Buffer(message.sender.host, "utf8");
    var senderPort = new Buffer(2);
    senderPort.writeUInt16BE(message.sender.port, 0);
...
function decodeDiscoveryMessage(buffer) {
  ...
  if (bufferComponents.length === 5 && bufferComponents[0][0] === 0) {
    ...
    result = {type: type, from: from, aid: aid, sender:{host:senderHost, port:senderPort}};
