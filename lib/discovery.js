...
function onDiscoveryMessage(buffer) {
  ...
  var message;
  console.log("Discovery buffer : ", buffer.toString('hex'));
  try {
...
function encodeDiscoveryMessage(message) {
  ...
  if (message.type === 'search') {
    ...
    result = concatDiscoveryBuffers([type, from, aid, senderHost, senderPort]);
    ...
    result = concatDiscoveryBuffers([type, from, to, aid, ip, pid, port]);
  }
...
function decodeDiscoveryMessage(buffer) {
  ...
  console.log("bufferComponents",bufferComponents);
 ...
 */
function concatDiscoveryBuffers(items) {
  var result = null;
  ...
  var offset = 0;
  _.forEach(items, function (item) {
    result.writeUInt16BE(item.length, offset);
    offset += 2;
