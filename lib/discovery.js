...
/**
 * Restart a discovery process for the provided actor in case last discovery
 * has been performed since more than `properties.cacheRefreshInterval` ms
 * This method is called in cased the provided actor is already in cache in order
 * to discover potential new instances of this actor.
 *
 * @param  {string} aid actor ID to refresh cache
 */
exports.maybeRefreshCache = function (aid) {
  aid = utils.aid.bare(aid);
  if (lastStoppedResearches[aid] && Date.now() - lastStoppedResearches[aid] >= properties.cacheRefreshInterval) {
    delete lastStoppedResearches[aid];
    exports.notifySearched(aid, true);
  }
}
 ...
 * @param aid {string}
 */
exports.notifySearched = function (aid, refreshCache) {
  aid = utils.aid.bare(aid);
  if (!lastResearches[aid]) {
    ...
    (function tick(aid) {
      if (started && Date.now() - lastResearches[aid] < properties.discoveryTimeout) {
        ...
        lastStoppedResearches[aid] = lastResearches[aid];
        delete lastResearches[aid];
        clearTimeout(researchesLoops[aid]);
