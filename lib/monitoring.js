 ...
 * Ping management
 */
application.addActor(properties.ID + '_ping', function (req) {
...
exports.pingContainer = function (cid, cb) {
  application.send(properties.ID, cid + '_ping', 'PING', properties.containerPingTimeout, function (err) {
    cb(err);
  });
};

/**
 * Container event loop round in ms
 * @returns {Number}
 */
exports.lag = function () {
  return toobusy.lag();
};

/**
 * check if event loop load is over threshold
 * @returns {Boolean}
 */
exports.toobusy = function () {
  return toobusy();
};
