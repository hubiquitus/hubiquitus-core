...
_.forEach(_.keys(levels), function (level) {
  Logger.prototype[level] = function () {
    ...
    if (level === 'error' || level === 'err' || level === 'warn') {
      messages.push({currentStack: new Error('Current stack')});
    }
    return internalLog(this, level, messages);
  };
});
...
var internalLog = function (logger, level, messages) {
  ...
  if (!_.isEmpty(errors)) messages.push({errors: errors});
  messages.push({container: {ID: properties.ID, name: properties.name, netInfo: properties.netInfo}});
...
function formatError(err) {
  return {err: ('Error ' + err.message), stack: err.stack};
}
