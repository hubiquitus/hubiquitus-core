...
function onRequest(socket, request) {
  ...
  filter(message, "request", function (err) {
    if (err) return logger.makeLog("trace", "hub-124", "request " + message.id + " filtered", err);
...
function onResponse(socket, response) {
  ...
  if (response.err) {
    ...
    filter(response.message, "response", function (err) {
      if (err) return logger.makeLog("trace", "hub-125", "response " + response.message.id + " filtered", err);
