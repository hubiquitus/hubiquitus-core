...
function onReq(err, sock, req) {
  ...
  if (toobusy()) {
    write(sock, {code: 'TOO_BUSY'}, req);
  } else if (actors.exists(req.to)) {
    ...
    write(sock, {code: 'ACTOR_NOT_FOUND'}, req);
  }
}
  ...
  if (err) {
    switch (err.code) {
      case 'ACTOR_NOT_FOUND':
        ...
        break;
      case 'TOO_BUSY':
        cache.disableContainer(sock.infos.cid);
        break;
    }
...
function searchReqSock(container) {
  ...
  if (sock) {
    ...
    sock = new Socket();
    sock.infos = {handler: onRes, cid: container.id};
