...
exports.start = function (done) {
  ...
  socketIn.on("message", function (socketBuffer, messageBuffer) {
    onZmqMessage(socketIn, messageBuffer);
  });
};
...
function onZmqMessage(socket, buffer) {
  ...
  if (ackFlag === ack.flag.ACK_NEEDED) {
    if (actors.exists(message.to)) {
      socket.send(protocol.buildZmqMessage(ack.flag.ACTOR_ACK, ackId));
      exports.onMessage(message, function (message) {
        socket.send(protocol.buildZmqMessage(ack.flag.NO_ACK, 0, message));
      });
    } else {
      socket.send(protocol.buildZmqMessage(ack.flag.CONTAINER_ACK, ackId));
    }
  } else {
...
function findSocketOut(container) {
  ...
  if (socketsOut[container.id]) {
    ...
    socket.on("message", function (socketBuffer, messageBuffer) {
      onZmqMessage(socket, messageBuffer);
    });
  }
  return socket;
}
