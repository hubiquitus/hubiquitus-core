 ...
 */
exports.flag = {
  ACK_NEEDED: 0,       // Protocol needs an ack within ack timeout
  NO_ACK: 1,           // Protocol doesn't need an ack. Used for reply in a request/reply
...
var onStatus = function (ackId, flag, aid, cid, cb) {
  ...
  if (flag === exports.flag.ACK_TIMEOUT) {
    err = logger.makeLog("trace", "hub-201", "ack timeout: container " + cid + " dead");
    actors.removeByContainer(cid);
  } else if (flag === exports.flag.CONTAINER_ACK) {
    err = logger.makeLog("trace", "hub-202", "ack response: actor " + aid + " dead");
    actors.remove(aid);
  } else if (flag === exports.flag.ACTOR_ACK) {
