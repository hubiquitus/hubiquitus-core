...
var onStatus = function (status, actor, cb) {
  ...
  if (status === exports.ackStatus.CONTAINER_NOT_FOUND) {
    err = logger.makeLog("trace", "hub-201", {msg: "ack timeout - container " + actor.container.id + " dead", ID: hubiquitus.ID});
    actors.removeAllByContainer(actor.container.id);
  } else if (status === exports.ackStatus.ACTOR_NOT_FOUND) {
    err = logger.makeLog("trace", "hub-202", {msg: "ack response - actor " + actor.aid + " dead", ID: hubiquitus.ID});
