...
exports.start = function (params, cb) {
  ...
  if (_properties['trace stats'] = (params.stats && params.stats['enabled'] === 'true')) {
...
exports.send = function (from, to, content, timeout, cb, headers) {
  ...
  if (started) {
    var req = {from: from, to: to, content: content, id: utils.uuid(), date: new Date().getTime(), headers: headers || {}};
...
function internalSend(req) {
  searchActor(req.to, function (aid) {
    if (new Date().getTime() < (req.date + req.timeout)) {
      ...
      if (_properties['trace stats']) setImmediate(stats.count, req.from, req.to);
    }
  });
}
...
function onReq(req, reply) {
  ...
  var mReply = function (err, content) {
    reply({from: actor.id, to: req.from, err: err, content: content, id: req.id});
  };
