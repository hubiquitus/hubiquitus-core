...
actors.on('actor removed', function (aid, scope) {
  _this.emit('actor removed', aid, scope);
});
...
function onReq(req, reply) {
  ...
  var mReply = function (err, content) {
    reply({from: actor.id, to: req.from, err: err, content: content, date: req.date, id: req.id, headers: req.headers});
  };
...
exports.addActor = function (aid, onMessage, scope) {
  ...
  var actor = scope || {};
  actor.id = aid;
  actor.container = {id: properties.ID, name: properties.ID, netInfo: properties.netInfo};
