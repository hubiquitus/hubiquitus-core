 ...
 * Node too busy setup
 */
toobusy.maxLag(properties.defaultMaxLag);
...
exports.start = function (params, cb) {
  ...
  if (!tv4.validate(params, schemas.startParams)) {
    ...
    _.isFunction(cb) && cb({code: 'TECHERR', cause: err});
    return this;
  }

  if (params.ip) properties.container.ip = params.ip;
...
exports.send = function (from, to, content, timeout, cb, headers) {
  ...
  if (started) {
    ...
    processMiddlewares(msgType.REQ_OUT, req, cb || utils.misc.idle, function () {
      if (cb) {
        var id = req.id; // avoid any changes to req.id
        events.once('res|' + id, function (res) {
          onRes(res, cb);
        });
        events.on('drop|' + id, function () {
          onDrop(req); // do not use the 'req' from the arguments to keep the original 'to'
        });
        setTimeout(function () {
          events.emit('res|' + id, {err: {code: 'TIMEOUT'}, id: id});
          events.removeAllListeners('drop|' + id);
        }, req.timeout);
      }
      internalSend(req);
    });
...
function internalSend(req) {
  var aid = req.to;
  searchActor(aid, req.timeout, function (cid) {
    if (Date.now() < (req.date + req.timeout)) {
      var container = cache.getContainer(cid);
      if (!container) return onDrop(req);
      if (actors.exists(aid)) { // if the actor is in the container
        ...
        adapters.remote.send(container, req);
      }
    }
  });
}
  ...
  processMiddlewares(msgType.REQ_IN, req, replyWrapper, function () {
    ...
    setImmediate(function () {
      try {
        ...
        var onMessage = actor.onMessage;
        onMessage(req); // use global scope
      } catch (err) {
...
exports.addActor = function (aid, onMessage) {
  if (!_.isString(aid) || !_.isFunction(onMessage)) {
    logger.makeLog('warn', 'hub-1', 'attempt to add an invalid actor !', {aid: aid, onMessage: onMessage});
    return this;
  }
