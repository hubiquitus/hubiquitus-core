...
exports.start = function (params, cb) {
  ...
  if (!started) {
    ...
    if (traceStats = (params.stats && params.stats['enabled'] === 'true')) setImmediate(stats.start, params.stats);
...
exports.send = function (from, to, content, timeout, cb) {
  ...
  if (started) {
    ...
    processMiddlewares(message, messageType.OUT, function () {
      if (_.isFunction(cb)) {
...
function onMessage(message, reply) {
  ...
  processMiddlewares(message, messageType.IN, function () {
      ...
      setImmediate(function () {
        try {
          actor.onMessage(message.from, message.payload.content, function (err, content) {
            ...
            processMiddlewares(response, messageType.REPLY_OUT, function () {
              reply(response);
            });
          });
        } catch (err) {
...
function onResponse(response) {
  ...
  processMiddlewares(response, messageType.REPLY_IN, function () {
 ...
 * @param cb {function} callback
 */
function processMiddlewares(message, type, cb) {
  var index = 0;
  var count = middlewares.length;
  (function next() {
    return (index < count) ? middlewares[index++](message, type, next) : (cb && cb());
  })();
}
