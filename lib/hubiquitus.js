...
var onMessage = function (message, cb) {
  logger.makeLog("trace", "hub-3", {msg: "message received", message: message, ID: ID});
  var actor = actors.get(message.to, actors.scope.PROCESS);
  if (!actor) {
    logger.makeLog("error", "hub-22", {msg: "actor not found", message: message, ID: ID});
  } else {
...
exports.start = function (params, cb) {
  ...
  if (!started) {
    ...
    startAdapters(function () {
      ...
      exports.netInfo = {ip: ip, pid: process.pid, ipc: adapters.ipc.addr, tcp: adapters.tcp.addr};
      logger.makeLog("info", "hub-18", {msg: "container started", ID: ID}, exports.netInfo);
      started = true;
...
exports.send = function (from, to, content, timeout, cb) {
  ...
  if (started) {
    ...
    if (utils.aid.isBare(from)) {
      var aid = actors.pick(from, actors.scope.PROCESS);
      if (!aid) {
        logger.makeLog("error", "hub-21", {msg: "actor not found", aid: from, ID: ID});
        return this;
      } else {
        from = aid;
      }
    ...
    findActor(to, function (aid) {
      ...
      if (scope === actors.scope.PROCESS) {
        ...
        logger.makeLog("trace", "hub-2", {msg: "message sent inproc", message: message, ID: ID});
      } else if (scope  === actors.scope.LOCAL) {
        adapters.ipc.send(actor.container, message);
        logger.makeLog("trace", "hub-14", {msg: "message sent ipc", message: message, ID: ID});
      } else if (scope === actors.scope.REMOTE) {
        adapters.tcp.send(actor.container, message);
        logger.makeLog("trace", "hub-15", {msg: "message sent tcp", message: message, ID: ID});
      }
