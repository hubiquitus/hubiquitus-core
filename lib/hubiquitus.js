...
exports.removeActor = function (aid) {
  ...
  actors.remove(aid, actors.scope.LOCAL);
  return this;
};
...
function searchActor(aid, cb) {
  ...
  (function _searchActor(aid) {
    var cacheAid = actors.pick(aid);
    if (cacheAid) events.emit(aid + '!found', cacheAid);
    discovery.notifySearched(aid, cacheAid ? null : function (aid) {
      events.emit(aid + '!found', cacheAid);
    });

    var count = researchesCount[aid];
    researchesCount[aid] = count ? ++count : 1;
    if (count < properties['max researches retry']) {
      setTimeout(function () {
        _searchActor(aid);
      }, properties['research timeout']);
    }
  })(aid);
}
