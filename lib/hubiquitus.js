...
exports.start = function (params, cb) {
  ...
  if (!started) {
    ...
    adapters.remote.start(function () {
      if (params.discoveryAddr || params.discoveryPort) discovery.start(params.discoveryAddr, params.discoveryPort, function () {
    ...
    findActor(to, function (aid) {
      if (!timeout || new Date().getTime() < (date + timeout)) {
        ...
        if (scope === actors.scope.PROCESS) {
          ...
          adapters.remote.send(actor.container, message, "tcp");
        }
...
adapters.inproc.on("message", onMessage);
adapters.remote.on("message", onMessage);
