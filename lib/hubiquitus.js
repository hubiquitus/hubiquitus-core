...
exports.send = function (from, to, content, timeout, cb) {
  ...
  if (started) {
    ...
    processMiddlewares(message, function (err) {
      if (err) return logger.makeLog("warn", "hub-31", "sending message " + message.id + "; middleware error", err);
      if (_.isFunction(cb)) {
...
function onMessage(message, cb) {
  ...
  processMiddlewares(message, function (err) {
    if (err) return logger.makeLog("warn", "hub-30", "on message " + message.id + "; middleware error", err);
...
function onResponse(response) {
  ...
  processMiddlewares(response, function (err) {
    if (err) return logger.makeLog("warn", "hub-32", "on response " + response.id + "; middleware error", err);
...
function processMiddlewares(message, cb) {
  ...
  (function next() {
    if (index < count) {
      ...
      middleware(message, function (err) {
        return err ? (cb && cb(err)) : next();
      });
    } else {
      cb && cb();
    }
