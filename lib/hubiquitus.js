...
function onDiscoverySearch(message) {
  ...
  var actors = pickAllActorsInCache(message.aid);
  if (!_.isEmpty(actors)) {
    logger.makeLog("trace", "hub-8", {msg: "actor " + message.aid + " found !", ID: ID});
    var netInfo = {/* TODO put ip, pid, port, ipc */};
    var response = {type: "result", from: ID, to: message.from, aid: message.aid, aids: actors, netInfo: netInfo};
...
function onDiscoveryResult(message) {
  ...
  _.forEach(message.aids, function (aid) {
    actors[aid] = {aid: aid, container: {id: message.from, netInfo: message.netInfo}};
  });
  var actor = message.aids[_.random(message.aids.length - 1)];
  events.emit(message.aid + "!found", actor);
}
 ...
 */
function pickAllActorsInCache(aid) {
  var actors = [];
  if (utils.aid.isFull(aid) && actors[aid] && actors[aid].container.id === ID) {
    actors.push(aid);
  } else {
