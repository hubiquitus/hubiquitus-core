...
exports.send = function (from, to, message, cb) {
  if (started) {
    ...
    startingQueue.push({from: from, to: to, message: message, cb: cb});
  }
  return this;
};
...
function processStartingQueue() {
  logger.makeLog("trace", "hub-17", {msg: "processing starting queue (" + startingQueue.length + " items)", ID: ID});
  _.forEach(startingQueue, function (message) {
    exports.send(message.from, message.to, message.message, message.cb);
  });
  startingQueue = [];
}
