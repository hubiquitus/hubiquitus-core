  ...
  if (started) {
    ...
    findActor(to, function (aid) {
      ...
      if (scope === actors.scope.PROCESS) {
        logger.makeLog("trace", "hub-2", {msg: "sending message inproc...", message: message, ID: ID});
        adapters.inproc.send(message);
      } else if (scope  === actors.scope.LOCAL) {
        logger.makeLog("trace", "hub-14", {msg: "sending message ipc...", message: message, ID: ID});
        adapters.ipc.send(actor.container, message);
      } else if (scope === actors.scope.REMOTE) {
        logger.makeLog("trace", "hub-15", {msg: "sending message tcp...", message: message, ID: ID});
