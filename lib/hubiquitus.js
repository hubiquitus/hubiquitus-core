...
adapters.axon.onMessage = onMessage;
    ...
    findActor(to, function (aid) {
      if (!timeout || new Date().getTime() < (date + timeout)) {
        ...
        if (scope === actors.scope.PROCESS) {
          ...
          adapters.tcp.send(actor.container, message); //TODO tcp is used temporarly
...
function collectNetInfo() {
  ...
  exports.netInfo.port = adapters.tcp.port;
}
