...
function onMessage(message, cb) {
  ...
  exports.filter(message, "request", function (err, valid) {
    if (err) return logger.makeLog("warn", "hub-30", "message " + message.id + " filtering error", err);
    if (!valid) return logger.makeLog("trace", "hub-31", "message " + message.id + " rejected");
...
function onResponse(response) {
  ...
  exports.filter(response, "request", function (err, valid) {
    if (err) return logger.makeLog("warn", "hub-32", "response " + response.id + " filtering error", err);
    if (!valid) return logger.makeLog("trace", "hub-33", "response " + response.id + " rejected");
 ...
 * @param type {string} message type (request|response)
 * @param cb {function} callback
 */
exports.filter = function (message, type, cb) {
  cb(null, true);
};
